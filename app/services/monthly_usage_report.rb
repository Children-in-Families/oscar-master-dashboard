class MonthlyUsageReport
  ...
  def aggregate_data_without_dev
    output = {
      total_instance: 0,
      user_count_all_time: 0,
      user_added_count: 0,
      user_deleted_count: 0,
      login_per_month: 0,
      client_count_all_time: 0,
    ...
    Organization.non_demo.find_each do |organization|
      ...
      output[:user_count_all_time] += data[:user_count_all_time]
      ...
      output[:client_count_all_time] += data[:client_count_all_time]
  ...
  def data_per_org(organization)
    Apartment::Tenant.switch(organization.short_name) do
...

      {
        ...
        user_count_all_time: User.non_devs.count,
        ...
        client_count_all_time: Client.count,
