class MonthlyUsageReport
  ...
  def aggregate_data_without_dev
    ...
    Organization.non_demo.where(created_at: beginning_of_month..end_of_month).find_each do |organization|
  ...
  def data_per_org(organization)
    Apartment::Tenant.switch(organization.short_name) do
...

      {
        ...
        user_count: User.non_devs.where(created_at: beginning_of_month..end_of_month).count,
        ...
        client_count: Client.where(created_at: beginning_of_month..end_of_month).count,
